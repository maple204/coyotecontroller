...
function compileExpr(expr) { 
  ...
  function evalKeyframes(kfs, tNorm, interp) {
    if (!kfs || kfs.length < 2) return { x: 0, y: 0 };
    let a = kfs[0], b = kfs[kfs.length - 1];
    for (let i = 0; i < kfs.length - 1; i++) { if (tNorm >= kfs[i].t && tNorm <= kfs[i + 1].t) { a = kfs[i]; b = kfs[i + 1]; break; } }
    let span = (b.t - a.t) || 1e-6; let u = clamp((tNorm - a.t) / span, 0, 1); if (interp === "smoothstep") u = u * u * (3 - 2 * u);
    return { x: lerp(a.x, b.x, u), y: lerp(a.y, b.y, u) };
  }

  async function loadMotionScripts() {
    try {
      const res = await fetch("motion_scripts.json", { cache: "no-store" }); if (!res.ok) return; const data = await res.json();
      motion.scripts = data.scripts || [];
      for (const s of motion.scripts) { if (s.type === "parametric") { s._fx = compileExpr(s.params.x); s._fy = compileExpr(s.params.y); } motion.byId.set(s.id, s); }
      refreshScriptSelectForMode(); refreshCaretakerSelect();
    } catch { }
  }
  ...
  function applyScriptToCursor(nowSec, dt) {
    motion.intensity = expSlew(motion.intensity, motion.intensityTarget, dt, 0.12); if (!motion.active || controlTargets.dragging || (gamepad.activeUntilSec && nowSec < gamepad.activeUntilSec)) return;
    const s = motion.byId.get(motion.scriptId); if (!s) return;
    const elapsed = nowSec - motion.startSec; const dur = s.duration_s || 10; let tNorm = (elapsed / dur); tNorm = s.loop ? (tNorm % 1.0) : clamp(tNorm, 0, 1);
    let x = 0, y = 0; if (s.type === "parametric") { x = s._fx(tNorm); y = s._fy(tNorm); } else { const k = evalKeyframes(s.keyframes, tNorm, s.interp); x = k.x; y = k.y; }
    controlTargets.x = clamp(x, -1, 1) * motion.intensity; controlTargets.y = clamp(y, -1, 1) * motion.intensity;
    const m = Math.hypot(controlTargets.x, controlTargets.y); if (m > 1) { controlTargets.x /= m; controlTargets.y /= m; }
    controlTargets.ax = controlTargets.x; controlTargets.ay = controlTargets.y;
  }
